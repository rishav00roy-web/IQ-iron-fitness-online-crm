...
export default function Header() {
  const { setIsAddOpen, setIsBroadcastOpen, setIsBroadcastAllOpen, setIsTrainersOpen, setIsSettingsOpen } = useCRM();
  return (
    <>
...
<button className="btn btn-primary" id="btn-add-member" onClick={() => setIsAddOpen(true)}>
...
<button className="btn btn-secondary" id="btn-broadcast-all" title="Broadcast to all visible members" onClick={() => setIsBroadcastAllOpen(true)}>
...
<button className="btn btn-secondary" id="btn-manage-trainers" onClick={() => setIsTrainersOpen(true)}>
...
<button className="btn btn-ghost" id="btn-export-csv" title="Export CSV">
...
<button className="btn btn-ghost" id="btn-import-trigger" title="Import CSV/JSON">
<svg fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="17 8 12 3 7 8"></polyline><line x1="12" x2="12" y1="3" y2="15"></line></svg>
        Import
      </button>
<input accept=".csv,.json" id="file-input" style={{display: 'none'}} type="file"/>
<button className="btn btn-icon" id="btn-settings" title="Settings" onClick={() => setIsSettingsOpen(true)}>
