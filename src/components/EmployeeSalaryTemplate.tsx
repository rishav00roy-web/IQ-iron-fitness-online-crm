...
export default function EmployeeSalaryTemplate({
  trainerName,
  basicPay,
  ...
  const commission =
    customCommission !== undefined
      ? customCommission
      : ptClients.reduce((sum, c) => sum + (c.pt_fee || 0), 0);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* HEADER SECTION (Deep Gold/Charcoal gradient) */}
        <div
          className="bg-gradient-to-r from-[#0a0d15] to-[#111722] text-white p-5 pr-12 rounded-t-lg flex justify-between items-center relative overflow-hidden shrink-0 z-10 shadow-md"
          style={{ padding: "20px", paddingRight: "48px" }}
        >
          ...
          <div className="relative z-10 flex flex-col items-end gap-2 pr-6" style={{ paddingRight: "24px" }}>
            <div className="bg-[#0a0d15] text-[#c8a96a] px-3 py-0.5 font-bold text-[10px] rounded-sm tracking-wider font-display uppercase border border-[#c8a96a]/30">
              SALARY SLIP
            </div>
            <div className="text-right text-[10px] space-y-0.5 font-mono text-slate-300 leading-tight">
              <div>
                Payslip No : <span className="font-bold text-white">{payslipNumber}</span>
              </div>
              <div>
                Date : <span className="text-white">{payslipDate}</span>
              </div>
              <div>
                Period : <span className="text-white">{period}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="my-4 shrink-0 z-10">
          <table className="w-full border-collapse border border-slate-200 text-[13px] table-fixed">
            <thead>
              <tr className="bg-[#111722] text-[#c8a96a] text-left font-display">
                <th className="py-2.5 px-3 border border-slate-300 w-[60%]" style={{ padding: "10px 12px" }}>
                  EARNINGS DESCRIPTION
                </th>
                <th className="py-2.5 px-3 border border-slate-300 w-[40%] text-right" style={{ padding: "10px 12px" }}>
                  AMOUNT ({currency})
                </th>
              </tr>
            </thead>
              ...
              <tr className="bg-white">
                <td className="py-4 px-3 border border-slate-200 align-top" style={{ padding: "16px 12px" }}>
                  ...
                  <span className="text-slate-500 text-[11px] block mt-0.5">
                    Sum of PT fees for {ptClients.length} assigned clients
                  </span>
                </td>
                <td className="py-4 px-3 border border-slate-200 text-right font-mono align-top" style={{ padding: "16px 12px" }}>
        ...
        {ptClients.length > 0 && (
          <div className="my-2 shrink-0 z-10">
            <h4 className="font-display font-bold text-[10px] uppercase tracking-wider text-[#c8a96a] mb-1">
        ...
        <div className="grid grid-cols-2 gap-6 mt-auto mb-4 shrink-0 z-10 pt-4">
          <div className="space-y-1">
            <div className="text-[10px] text-slate-500 italic mt-1 leading-snug p-3 bg-slate-50 border border-slate-100 rounded">
              <span className="font-bold text-slate-700 block not-italic uppercase tracking-wide text-[8px] mb-1">
                Net Pay in Words:
              </span>
              <span>{numberToWords(netSalary, currency)}</span>
            </div>
          </div>

          {/* TOTALS SUMMARY BLOCK */}
          <div className="space-y-2 text-[12px] font-body flex flex-col justify-end">
            {/* Net Salary Ribbon (Gold background with charcoal text) */}
            <div
              className="bg-gradient-to-r from-[#c8a96a] to-[#e2c990] text-[#0a0d15] py-2.5 px-4 rounded-sm flex justify-between items-center shadow-md relative overflow-hidden mt-2"
            ...
            >
              ...
              <span className="font-mono text-lg font-black">{formatCurrency(netSalary)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-6 items-end my-3 pt-3 border-t border-slate-200/50 shrink-0 font-body z-10">
          <div className="col-span-2 space-y-1.5">
            <h5 className="font-display font-bold text-[#c8a96a] tracking-wider uppercase text-[9px]">Notes</h5>
            <ul className="list-disc list-inside text-[9px] text-slate-500 space-y-0.5 leading-relaxed">
              ...
              <li>All amounts are in local settings currency.</li>
            </ul>
          </div>
